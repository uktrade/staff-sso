...
class TestShowUserPermissionsView:
    ...
    def test_application_permissions(self, rf):
        admin_user = UserFactory(
            first_name="admin", last_name="admin", is_superuser=True, is_staff=True
        )
        user = UserFactory(first_name="Bill", last_name="Smith")
...
class TestMergeAction:
    def test_require_more_than_one_user(self, auth_client):
        ...
        assert (
            b'<li class="error">You need to select at least two records to use the merge function</li>'
            in response.content
        )  # noqa: E501
    ...
    def test_view_merge_form(self, auth_client):
        ...
        assert set(matches) == set(
            [f'<input type="hidden" name="_selected_action" value="{id}">' for id in ids]
        )

        matches = re.findall(
            r'<input type="radio" name="merge-primary-id" value="\d*" required>', content
        )
        assert set(matches) == set(
            [f'<input type="radio" name="merge-primary-id" value="{id}" required>' for id in ids]
        )
    ...
    def test_successful_merge(self, auth_client):
        ...
        user3 = UserFactory(
            email="user3_1@email.com", email_list=["user3_2@email.com", "user3_3@email.com"]
        )
        ...
        auth_client.post(
            reverse("admin:user_user_changelist"),
            {
                ...
                "merge-primary-id": primary_user.pk,
                "post": "yes",
            },
            follow=True,
        )
        ...
        all_emails = set(
            [email.email for user in [primary_user, user2, user3] for email in user.emails.all()]
        )
    ...
    def test_deleted_users_are_logged(self, auth_client):
        ...
        assert (
            log.object_repr
            == f"User(id={user2.pk}, user_id={user2.user_id}, email_user_id={user2.email_user_id})"
        )
        assert log.change_message == f"merged into {primary_user.user_id}"

    @pytest.mark.parametrize(
        "perms,expected_status",
        [
            ([], 403),
            (["change_user"], 403),
            (["delete_user"], 403),
