...
class TestUserSettings:
    MY_USER_SETTINGS_PER_APP_URL = reverse_lazy("api-v1:user-settings:settings_per_app")
    ...
    def test_update(self, api_client):
        ...
        UserSettingsFactory.create_batch(5, user_id=user.user_id)
        UserSettingsFactory.create(
            user_id=user.user_id, app_slug="Test oauth app", settings="private_cake: Carrot cake"
        )
    ...
    def test_update_nested_items_incorrectly(self, api_client):
            ...
            "Test oauth app": {
                "private_cake": {
                    "multi_layer": {
                        ...
                        "fourth_layer": {"frosting": "whipped_cream", "sprinkles": "strawberries"},
                        "second_layer": "cream",
        ...
        response = api_client.post(
            "/api/v1/user-settings/",
            {"@": {"private_cake": {"multi_layer": {"first_layer": {"support": "plate"}}}}},
            format="json",
        )
    ...
    def test_get(self, api_client):
        ...
        UserSettingsFactory.create(
            user_id=user.user_id, app_slug="global", settings="global_cake: Coffee and walnut cake"
        )
    ...
    def test_delete_item(self, api_client):
        ...
        user_settings_before = UserSettings.objects.get(
            user_id=user.user_id, app_slug="Test oauth app"
        )

        api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)

        response = api_client.delete(
            "/api/v1/user-settings/", {"@": {"private_cake": {}}}, format="json"
        )

        try:
            user_settings_after = UserSettings.objects.get(
                user_id=user.user_id, app_slug="Test oauth app"
            )
    ...
    def test_delete_nonexistent_item(self, api_client):
        ...
        response = api_client.delete("/api/v1/user-settings/", {"@": {"brick": {}}}, format="json")
    ...
    def test_delete_nested_items(self, api_client):
        ...
        response = api_client.delete(
            "/api/v1/user-settings/",
            {"@": {"private_cake": {"multi_layer": {"fourth_layer": {"sprinkles": {}}}}}},
            format="json",
        )
        assert response.status_code == 204
