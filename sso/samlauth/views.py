...
#     get_location,
# )
# from djangosaml2.views import _get_subject_id_set_subject_id, AssertionConsumerServiceView, finish_logout, LoginView
# from djangosaml2.signals import post_authenticated

from djangosaml2.views import AssertionConsumerServiceView, finish_logout, LoginView
...
class CustomAssertionConsumerServiceView(AssertionConsumerServiceView):
    def post(self, request, attribute_mapping=None, create_unknown_user=None):
        ...
        http_response = super().post(
            request, attribute_mapping=None, create_unknown_user=None
        )
        # email = self._extract_email_from_session_info()

        # remember the email the user entered so that we can pre-populate the login start form
    ...
    def post_login_hook(
        ...
        if session_info["issuer"] in getattr(
