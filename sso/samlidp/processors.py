...
class ModelProcessor(BaseProcessor):
    ...
    def has_access(self, request):
        ...
        access = (
            request.user.can_access(self._application)
            and self._application.active
            and self._application.is_valid_ip(request)
        )

        create_x_access_log(
            request, 200 if access else 403, application=self._application.pretty_name
        )
    ...
    def create_identity(self, user, sp_attribute_mapping: Dict[str, str]) -> Dict[str, str]:
        ...
        try:
            ...
            raise ImproperlyConfigured(
                "AWS processor requires a role to be provided in extra_config"
            )

        identity["https://aws.amazon.com/SAML/Attributes/RoleSessionName"] = self.get_user_id(
            user, None, None, None
        )
...
class ApplicationPermissionProcessor(ModelProcessor):
    ...
    def create_identity(self, user, sp_mapping, **extra_config):
        ...
        permissions = list(
            user.application_permissions.filter(saml2_application=self._application).values_list(
