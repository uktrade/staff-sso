...
class EmailForm(forms.Form):
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control form-control-1-4", "placeholder": "i.e. john.smith5"}
        ),
    )
    ...
    def clean(self):
        ...
        validate_email = EmailValidator("Enter the first part of your email address only")
    ...
    def send_signin_email(self, request):
        ...
        url = "{scheme}{host}{path}?next={next_url}".format(
            scheme="https://", host=request.get_host(), path=path, next_url=next_url
        )
