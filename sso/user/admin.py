...
class UserForm(ModelForm):
    ...
    def save(self, *args, **kwargs):
        ...
        in the `became_inactive_on` field."""

        kwargs["commit"] = False
        object = super().save(*args, **kwargs)

        if "is_active" in self.changed_data:
            if not self.cleaned_data["is_active"] and not object.became_inactive_on:
                object.became_inactive_on = timezone.now()
            if self.cleaned_data["is_active"] and object.became_inactive_on:
                object.became_inactive_on = None
...
class UserAdmin(admin.ModelAdmin):
    ...
    list_display = (
        ...
        "last_login",
        "last_accessed",
        "list_permitted_applications",
        "list_access_profiles",
        "show_permissions_link",
    )
    inlines = [
        EmailInline,
        ServiceEmailInline,
    ...
    def show_permissions_link(self, obj):
        return mark_safe(
            '<a href="{}" target="_blank">show perms</a>'.format(
    ...
    def merge_users(self, request, queryset):
        ...
        perms_needed = not (
            request.user.has_perm("user.delete_user") and request.user.has_perm("user.change_user")
        )
        ...
        return TemplateResponse(request, self.merge_users_confirmation_template, context)
