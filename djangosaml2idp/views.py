...
class LoginProcessView(LoginRequiredMixin, IdPHandlerViewMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #from saml2 import NAMEID_FORMAT_EMAIL
        # Will this break aws integration?
        resp_args['name_id_policy'].format = "urn:oasis:names:tc:SAML:2.0:nameid-format:email"

        print('sign assertion {}'.format('true' if self.IDP.config.getattr("sign_assertion", "idp") else 'false'))

        # Construct SamlResponse message
        try:
            authn_resp = self.IDP.create_authn_response(
                identity=identity, userid=request.user.username,
                name_id=NameID(format=resp_args['name_id_policy'].format, sp_name_qualifier=resp_args['destination'],
